...
The library is installed as a single file, e.g. :file:`libscic-errno.a` for the :file:`scic/errno` library.  A shared
version of the library :file:`libscic-errno.so` is also installed on systems
